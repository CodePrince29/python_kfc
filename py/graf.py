   ...
   def graf( self, path, fecha ):
      ...
      if exists.exists( fullname ):
         try: 
            ...
            datos = []
            for li in lines:
               datos.append( map(string.strep, string.split(li,",") ) )
            cc = chart.CCharts()
            cc.loadDefiniciones("/home/httpd/cgi-bin/py/FORMATOS/" + self.directorio + ".grf" )
            cc.getLabels( self, datos)
            cc.getDatos(self, datos)
...
class CReports:
   ...
   def toGRAF(self, fecha, year, periodo, semana, lista): 
      myhtml.header()
      for reporte in lista :
         if self.dict.has_key(reporte):
            ...
            self.dict[reporte].graf( self.path, fecha )
      myhtml.footer()
...
def main():
   ...
   k.getCGIparms()
   if len( k.reports ):
      ctgRpts.toGRAF( k.dia, k.year, k.periodo, k.semana, k.reports )
...
#k.dia = "99-03-08"
#k.reports = [ "splan" ]
ctgRpts.toGRAF( "99-03-09", "99", "04","01", [ "sales" ] )
##########################################
